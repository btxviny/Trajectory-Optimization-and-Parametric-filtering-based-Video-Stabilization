...
def optimize_path(flows):
    ...
    for step in range(ITERATIONS):
        ...
        print(f'\rStep {step}\{ITERATIONS}, Loss: {loss.item()}',end='')
        warps = (smooth_trajectories - original_trajectories).detach()
        warps = gaussian_blur(warps.permute(0,3,1,2),kernel_size=5,sigma=1).cpu()
        return warps.permute(0,2,3,1).numpy()
